...
class medt_net(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        # x = F.relu(F.interpolate(self.decoder1(x4), scale_factor=(2,2), mode ='bilinear', align_corners=True))
        # x = torch.add(x, x4)
        # x = F.relu(F.interpolate(self.decoder2(x4) , scale_factor=(2,2), mode ='bilinear', align_corners=True))
        # x = torch.add(x, x3)
        # x = F.relu(F.interpolate(self.decoder3(x3) , scale_factor=(2,2), mode ='bilinear', align_corners=True))
        # x = torch.add(x, x2)
